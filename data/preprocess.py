...
def load_prices(input_dir):
    ...
    print(
        f"\n✔  Loaded prices for {prices_df.shape[1]} assets "
        f"over {prices_df.shape[0]} trading days."
    )
...
def plot_price_evolution(prices_df, results_dir):
    ...
    ax.set_title(
        "Normalized Price Evolution (Base 100) – Bank Nifty Stocks (2 Years)",
        fontsize=14,
        fontweight="bold",
        pad=15,
    )
...
def plot_return_distributions(returns_df, results_dir):
    ...
    plt.suptitle(
        "Return Distribution Analysis – Bank Nifty Stocks",
        fontsize=14,
        fontweight="bold",
        y=1.00,
    )
...
def plot_correlation_heatmap(corr, results_dir):
    ...
    sns.heatmap(
        ...
        cmap=cmap,
        vmin=-1,
        vmax=1,  # fix scale to the full −1…+1 range
